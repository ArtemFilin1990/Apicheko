...
test("risk scoring applies low-only penalty for a single 0₽ defendant case in 24m", async () => {
  const riskModule = await import(`${pathToFileURL(riskSourcePath).href}?v=${Date.now()}-zero`);
  const payload = {
    companyData: { Статус: { Наим: "Действующее" }, ДатаРег: "2018-01-01", Налоги: { СумНедоим: 0 }, Контакты: { Тел: ["+7 999"], Емэйл: ["a@test.ru"] } },
    financesData: { "2023": { 2110: 3000000, 2400: 200000 } },
    legalData: [{ role: "defendant", date: new Date().toISOString(), amount: 0 }],
    fsspData: [],
    bankruptcyData: [],
    fedresursData: [],
    dadataParty: { employee_count: 12, invalid: false }
  };

  const result = riskModule.calculateCompanyRiskScore(payload);
  const defendantFactor = result.factors.find((factor) => factor.code === "DEFENDANT_CASES_24M_LOW");
  assert.ok(defendantFactor);
  assert.equal(defendantFactor.points, -1);
  assert.ok(result.decision !== "manual_review" || result.level !== "high");
});

test("risk scoring ignores ambiguous defendant role for penalty", async () => {
  const riskModule = await import(`${pathToFileURL(riskSourcePath).href}?v=${Date.now()}-amb`);
  const payload = {
    companyData: { Статус: { Наим: "Действующее" }, ДатаРег: "2015-01-01", Налоги: { СумНедоим: 0 }, Контакты: { Тел: ["+7 999"] } },
    financesData: { "2023": { 2110: 2500000, 2400: 140000 } },
    legalData: [{ description: "В тексте упомянут ответчик", date: new Date().toISOString(), amount: 120000 }],
    fsspData: [],
    bankruptcyData: [],
    fedresursData: [],
    dadataParty: { employee_count: 15, invalid: false }
  };

  const result = riskModule.calculateCompanyRiskScore(payload);
  assert.equal(result.factors.some((factor) => /^DEFENDANT_CASES_24M/.test(factor.code)), false);
});

test("risk scoring raises defendant medium/high pressure and debt compound", async () => {
  const riskModule = await import(`${pathToFileURL(riskSourcePath).href}?v=${Date.now()}-def-hi`);
  const nowIso = new Date().toISOString();
  const payload = {
    companyData: { Статус: { Наим: "Действующее" }, ДатаРег: "2012-01-01", Налоги: { СумНедоим: 450000 } },
    legalData: [
      { role: "defendant", date: nowIso, amount: 800000, claimant: "A" },
      { role: "respondent", date: nowIso, amount: 700000, claimant: "B" },
      { role: "ответчик", date: nowIso, amount: 900000, claimant: "C" },
      { role: "defendant", date: nowIso, amount: 600000, claimant: "D" },
      { role: "defendant", date: nowIso, amount: 500000, claimant: "E" },
      { role: "defendant", date: nowIso, amount: 400000, claimant: "F" }
    ],
    fsspData: [{ id: 1 }],
    dadataParty: { employee_count: 4 }
  };

  const result = riskModule.calculateCompanyRiskScore(payload);
  assert.ok(result.factors.some((factor) => factor.code === "DEFENDANT_CASES_24M_HIGH"));
...
test("risk scoring avoids legal-load double penalty on pure defendant pool", async () => {
  const riskModule = await import(`${pathToFileURL(riskSourcePath).href}?v=${Date.now()}-nodouble`);
  const nowIso = new Date().toISOString();
  const legalData = Array.from({ length: 8 }, (_, i) => ({ role: "defendant", date: nowIso, amount: 200000 + i * 1000 }));
  const result = riskModule.calculateCompanyRiskScore({
    companyData: { Статус: { Наим: "Действующее" }, ДатаРег: "2016-01-01", Налоги: { СумНедоим: 0 } },
    legalData,
    fsspData: [],
    dadataParty: { employee_count: 8 }
  });

  assert.ok(result.factors.some((factor) => factor.code === "DEFENDANT_CASES_24M_HIGH"));
  assert.equal(result.factors.some((factor) => factor.code === "LEGAL_LOAD_HIGH" || factor.code === "LEGAL_LOAD_MEDIUM"), false);
});

test("missing finance for very young company does not force high risk", async () => {
  const riskModule = await import(`${pathToFileURL(riskSourcePath).href}?v=${Date.now()}-young`);
  const recentDate = new Date(Date.now() - 120 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const result = riskModule.calculateCompanyRiskScore({
    companyData: { Статус: { Наим: "Действующее" }, ДатаРег: recentDate, Контакты: { Тел: ["+7 123"] } },
    legalData: [],
    fsspData: [],
    dadataParty: { employee_count: 2 }
  });

  assert.equal(result.factors.some((factor) => factor.code === "FINANCE_GAP"), false);
  assert.ok(["low", "medium"].includes(result.level));
});

test("telegram risk formatter remains compact and valid", async () => {
  const riskModule = await import(`${pathToFileURL(riskSourcePath).href}?v=${Date.now()}-fmt`);
  const result = riskModule.calculateCompanyRiskScore({
    companyData: { Статус: { Наим: "Действующее" }, ДатаРег: "2010-01-01", Налоги: { СумНедоим: 0 }, Контакты: { Тел: ["+7 000"] } },
    financesData: { "2023": { 2110: 3000000, 2400: 100000 } },
    legalData: [],
    fsspData: [],
    dadataParty: { employee_count: 10 }
  });
